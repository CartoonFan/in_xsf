...
// This gets the directory for the path, including the final forward/backward
// slash
template <typename T>
inline std::basic_string<T>
ExtractDirectoryFromPath(const std::basic_string<T> &fullPath) {
    auto lastSlash = fullPath.rfind('\\');
    if (lastSlash == std::basic_string<T>::npos)
        lastSlash = fullPath.rfind('/');
    return lastSlash != std::basic_string<T>::npos
...
template <typename T>
inline std::basic_string<T>
ExtractFilenameFromPath(const std::basic_string<T> &fullPath) {
    auto lastSlash = fullPath.rfind('\\');
    if (lastSlash == std::basic_string<T>::npos)
        lastSlash = fullPath.rfind('/');
    return lastSlash != std::basic_string<T>::npos
...
template <typename T> inline T NextHighestPowerOf2(T value) {
    if (value < 1)
        return 1;
    --value;
    for (size_t i = 1; i < sizeof(T) * std::numeric_limits<unsigned char>::digits;
            i <<= 1)
        value |= value >> i;
    return value + 1;
}
...
inline void clamp(T1 &valueToClamp, const T2 &minValue, const T2 &maxValue) {
