...
struct SoundViewData {
    ...
    SoundViewData()
        : config(nullptr), player(nullptr), hDlg(nullptr),
          volModeAlternative(false) {
        std::fill_n(&this->channelLastStates[0], sizeof(this->channelLastStates),
                    CS_START);
    }
};
#endif
...
protected:
    ...
    static INT_PTR CALLBACK SoundViewDialogProc(HWND hwndDlg, UINT uMsg,
...
#ifdef _DEBUG
    void CallSoundView(XSFPlayer *xSFPlayer, HINSTANCE hInstance,
                       HWND hwndParent);
