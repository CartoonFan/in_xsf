...
public:
    ...
    RelativePositionToParent *Clone() const {
        return new RelativePositionToParent(this->relativePosition, this->positionType);
    }
};
...
public:
    ...
    RelativePositionToSibling *Clone() const {
        return new RelativePositionToSibling(this->relativePosition, this->positionType, this->siblingsBack);
    }
};
...
public:
    ...
    DialogBuilder &WithFont(const std::wstring &FontName, uint16_t FontSizeInPts) {
    ...
    DialogBuilder &WithBorder(bool Border = true) {
        if (Border) this->style |= WS_BORDER;
        else this->style &= ~WS_BORDER;
        return *this;
    }
    DialogBuilder &WithDialogFrame(bool DialogFrame = true) {
        if (DialogFrame) this->style |= WS_DLGFRAME;
        else this->style &= ~WS_DLGFRAME;
        return *this;
    }
    DialogBuilder &WithVerticalScrollbar(bool VerticalScrollbar = true) {
        if (VerticalScrollbar) this->style |= WS_VSCROLL;
        else this->style &= ~WS_VSCROLL;
        return *this;
    }
    DialogBuilder &WithHorizontalScrollbar(bool HorizontalScrollbar = true) {
        if (HorizontalScrollbar) this->style |= WS_HSCROLL;
        else this->style &= ~WS_HSCROLL;
        return *this;
    }
    DialogBuilder &WithSystemMenu(bool SystemMenu = true) {
        if (SystemMenu) this->style |= WS_SYSMENU;
        else this->style &= ~WS_SYSMENU;
        return *this;
    }
    DialogBuilder &WithSizingBorder(bool SizingBorder = true) {
        if (SizingBorder) this->style |= WS_THICKFRAME;
        else this->style &= ~WS_THICKFRAME;
        return *this;
    }
    DialogBuilder &WithMinimizeBox(bool MinimizeBox = true) {
        if (MinimizeBox) this->style |= WS_MINIMIZEBOX;
        else this->style &= ~WS_MINIMIZEBOX;
        return *this;
    }
    DialogBuilder &WithMaximizeBox(bool MaximizeBox = true) {
        if (MaximizeBox) this->style |= WS_MAXIMIZEBOX;
        else this->style &= ~WS_MAXIMIZEBOX;
        return *this;
    }
    DialogBuilder &WithDialogModalFrame(bool DialogModalFrame = true) {
        if (DialogModalFrame) this->exstyle |= WS_EX_DLGMODALFRAME;
        else this->exstyle &= ~WS_EX_DLGMODALFRAME;
        return *this;
    }
#if WINVER >= 0x0400
    DialogBuilder &WithRaisedEdge(bool RaisedEdge = true) {
        if (RaisedEdge) this->exstyle |= WS_EX_WINDOWEDGE;
        else this->exstyle &= ~WS_EX_WINDOWEDGE;
        return *this;
    }
    DialogBuilder &WithSunkenEdge(bool SunkenEdge = true) {
        if (SunkenEdge) this->exstyle |= WS_EX_CLIENTEDGE;
        else this->exstyle &= ~WS_EX_CLIENTEDGE;
        return *this;
    }
    DialogBuilder &WithContextHelp(bool ContextHelp = true) {
        if (ContextHelp) this->exstyle |= WS_EX_CONTEXTHELP;
        else this->exstyle &= ~WS_EX_CONTEXTHELP;
        return *this;
    }
    DialogBuilder &IsControlWindow(bool ControlWindow = true) {
        if (ControlWindow) this->style |= DS_CONTROL;
        else this->style &= ~DS_CONTROL;
        return *this;
    }
    DialogBuilder &IsCentered(bool Centered = true) {
        if (Centered) this->style |= DS_CENTER;
        else this->style &= ~DS_CENTER;
        return *this;
    }
#endif
};
...
public:
    ...
    T &IsDisabled(bool Disabled = true) {
        if (Disabled) this->style |= WS_DISABLED;
        else this->style &= ~WS_DISABLED;
        return this->me();
    }
    T &WithBorder(bool ThinBorder = true) {
        if (ThinBorder) this->style |= WS_BORDER;
        else this->style &= ~WS_BORDER;
        return this->me();
    }
    ...
    T &WithTabStop(bool TabStop = true) {
        if (TabStop) this->style |= WS_TABSTOP;
        else this->style &= ~WS_TABSTOP;
        return this->me();
    }
#if WINVER >= 0x0400
...
public:
    ...
    DialogEditBoxBuilder &IsMultiline(bool Multiline = true) {
        if (Multiline) this->style |= ES_MULTILINE;
        else this->style &= ~ES_MULTILINE;
        return this->me();
    }
    DialogEditBoxBuilder &WithAllUppercase(bool AllUppercase = true) {
        if (AllUppercase) this->style |= ES_UPPERCASE;
        else this->style &= ~ES_UPPERCASE;
        return this->me();
    }
    DialogEditBoxBuilder &WithAllLowercase(bool AllLowercase = true) {
        if (AllLowercase) this->style |= ES_LOWERCASE;
        else this->style &= ~ES_LOWERCASE;
        return this->me();
    }
    DialogEditBoxBuilder &IsPasswordInput(bool PasswordInput = true) {
        if (PasswordInput) this->style |= ES_PASSWORD;
        else this->style &= ~ES_PASSWORD;
        return this->me();
    }
    DialogEditBoxBuilder &WithAutoVScroll(bool AutoVScroll = true) {
        if (AutoVScroll) this->style |= ES_AUTOVSCROLL;
        else this->style &= ~ES_AUTOVSCROLL;
        return this->me();
    }
    DialogEditBoxBuilder &WithAutoHScroll(bool AutoHScroll = true) {
        if (AutoHScroll) this->style |= ES_AUTOHSCROLL;
        else this->style &= ~ES_AUTOHSCROLL;
        return this->me();
    }
    DialogEditBoxBuilder &WithDontHideSelection(bool DontHideSelection = true) {
        if (DontHideSelection) this->style |= ES_NOHIDESEL;
        else this->style &= ~ES_NOHIDESEL;
        return this->me();
    }
    DialogEditBoxBuilder &IsReadOnly(bool ReadOnly = true) {
        if (ReadOnly) this->style |= ES_READONLY;
        else this->style &= ~ES_READONLY;
        return this->me();
    }
    DialogEditBoxBuilder &WithWantReturn(bool WantReturn = true) {
        if (WantReturn) this->style |= ES_WANTRETURN;
        else this->style &= ~ES_WANTRETURN;
        return this->me();
    }
};
...
public:
    ...
    DialogLabelBuilder &WithAmpsNotTranslated(bool AmpsNotTranslated = true) {
        if (AmpsNotTranslated) this->style |= SS_NOPREFIX;
        else this->style &= ~SS_NOPREFIX;
        return this->me();
    }
#if WINVER >= 0x0400
    DialogLabelBuilder &WithNotify(bool Notify = true) {
        if (Notify) this->style |= SS_NOTIFY;
        else this->style &= ~SS_NOTIFY;
        return this->me();
    }
#endif
};
...
public:
    ...
    T &IsCenterJustified(bool CenterJustified = true) {
        if (CenterJustified) this->style |= BS_CENTER;
        else this->style &= ~BS_CENTER;
        return this->me();
    }
    ...
    T &WithVerticalCenter(bool VerticalCenter = true) {
        if (VerticalCenter) this->style |= BS_VCENTER;
        else this->style &= ~BS_VCENTER;
        return this->me();
    }
    T &IsMultiline(bool Multiline = true) {
        if (Multiline) this->style |= BS_MULTILINE;
        else this->style &= ~BS_MULTILINE;
        return this->me();
    }
    T &WithNotify(bool Notify = true) {
        if (Notify) this->style |= BS_NOTIFY;
        else this->style &= ~BS_NOTIFY;
        return this->me();
    }
    T &IsFlat(bool Flat = true) {
        if (Flat) this->style |= BS_FLAT;
        else this->style &= ~BS_FLAT;
        return this->me();
    }
#endif
};
...
public:
    ...
    DialogButtonBuilder &IsDefault(bool Default = true) {
        if (Default) this->style |= BS_DEFPUSHBUTTON;
        else this->style &= ~BS_DEFPUSHBUTTON;
        return this->me();
    }
};
...
protected:
    friend class DialogTemplate;
public:
    DialogCheckBoxBuilder(const std::wstring &Label) : DialogButtonBaseBuilder(CHECKBOX_CONTROL, Label) {
        this->style |= BS_AUTOCHECKBOX;
    }
    DialogCheckBoxBuilder &WithTextOnLeft(bool TextOnLeft = true) {
        if (TextOnLeft) this->style |= BS_LEFTTEXT;
        else this->style &= ~BS_LEFTTEXT;
        return this->me();
    }
    DialogCheckBoxBuilder &LikePushButton(bool PushButton = true) {
        if (PushButton) this->style |= BS_PUSHLIKE;
        else this->style &= ~BS_PUSHLIKE;
        return this->me();
    }
};
...
public:
    ...
    DialogListBoxBuilder &WithNotify(bool Notify = true) {
        if (Notify) this->style |= LBS_NOTIFY;
        else this->style &= ~LBS_NOTIFY;
        return this->me();
    }
    DialogListBoxBuilder &WithSort(bool Sort = true) {
        if (Sort) this->style |= LBS_SORT;
        else this->style &= ~LBS_SORT;
        return this->me();
    }
    DialogListBoxBuilder &WithMultipleSelect(bool MultipleSelect = true) {
        if (MultipleSelect) this->style |= LBS_MULTIPLESEL;
        else this->style &= ~LBS_MULTIPLESEL;
        return this->me();
    }
    DialogListBoxBuilder &WithExactHeight(bool ExactHeight = true) {
        if (ExactHeight) this->style |= LBS_NOINTEGRALHEIGHT;
        else this->style &= ~LBS_NOINTEGRALHEIGHT;
        return this->me();
    }
    DialogListBoxBuilder &WithMultipleColumns(bool MultipleColumns = true) {
        if (MultipleColumns) this->style |= LBS_MULTICOLUMN;
        else this->style &= ~LBS_MULTICOLUMN;
        return this->me();
    }
    DialogListBoxBuilder &WithExtendedSelect(bool ExtendedSelect = true) {
        if (ExtendedSelect) this->style |= LBS_EXTENDEDSEL;
        else this->style &= ~LBS_EXTENDEDSEL;
        return this->me();
    }
    DialogListBoxBuilder &WithDisabledNoScroll(bool DisabledNoScroll = true) {
        if (DisabledNoScroll) this->style |= LBS_DISABLENOSCROLL;
        else this->style &= ~LBS_DISABLENOSCROLL;
        return this->me();
    }
#if WINVER >= 0x0400
    DialogListBoxBuilder &WithNoSelect(bool NoSelect = true) {
        if (NoSelect) this->style |= LBS_NOSEL;
        else this->style &= ~LBS_NOSEL;
        return this->me();
    }
#endif
};
...
public:
    ...
    DialogComboBoxBuilder &WithAutoHScroll(bool AutoHScroll = true) {
        if (AutoHScroll) this->style |= CBS_AUTOHSCROLL;
        else this->style &= ~CBS_AUTOHSCROLL;
        return this->me();
    }
    DialogComboBoxBuilder &WithSort(bool Sort = true) {
        if (Sort) this->style |= CBS_SORT;
        else this->style &= ~CBS_SORT;
        return this->me();
    }
    DialogComboBoxBuilder &WithExactHeight(bool ExactHeight = true) {
        if (ExactHeight) this->style |= CBS_NOINTEGRALHEIGHT;
        else this->style &= ~CBS_NOINTEGRALHEIGHT;
        return this->me();
    }
    DialogComboBoxBuilder &WithDisabledNoScroll(bool DisabledNoScroll = true) {
        if (DisabledNoScroll) this->style |= CBS_DISABLENOSCROLL;
        else this->style &= ~CBS_DISABLENOSCROLL;
        return this->me();
    }
#if WINVER >= 0x0400
    DialogComboBoxBuilder &WithAllLowercase(bool AllLowercase = true) {
        if (AllLowercase) this->style |= CBS_LOWERCASE;
        else this->style &= ~CBS_LOWERCASE;
        return this->me();
    }
    DialogComboBoxBuilder &WithAllUppercase(bool AllUppercase = true) {
        if (AllUppercase) this->style |= CBS_UPPERCASE;
        else this->style &= ~CBS_UPPERCASE;
        return this->me();
    }
#endif
};
...
class DialogTemplate
{
    ...
    protected:
        ...
        {
            std::for_each(control.controls.begin(), control.controls.end(), [&](const std::unique_ptr<DialogControl> &ctl) {
                this->controls.push_back(std::unique_ptr<DialogControl>(ctl->Clone()));
            });
        }
    ...
    public:
        ...
        short GetControlHeight() const {
            return (this->style & CBS_SIMPLE && !(this->style & CBS_DROPDOWNLIST)) ? this->rect.size.height : 14;
        }
    };
...
public:
    ...
    {
        std::for_each(dlg.controls.begin(), dlg.controls.end(), [&](const std::unique_ptr<DialogControl> &control) {
            this->controls.push_back(std::unique_ptr<DialogControl>(control->Clone()));
        });
    }
