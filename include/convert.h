 ...
 *
 * The convert and convertTo functions were made into templates of
 * std::basic_string to handle wide-character strings properly, as well as
 * adding wstringify for the same reason.
 */
...
inline void convert(const std::basic_string<S> &s, T &x,
...
template <typename T, typename S>
inline T convertTo(const std::basic_string<S> &s,
                   bool failIfLeftoverChars = true) {
    T x;
...
private:
    template <typename T>
    static bool IsDigitsOnly(const std::basic_string<T> &input,
        ...
        std::use_facet<std::ctype<T>>(loc).is(&inputChars[0], &inputChars[length],
                                              &masks[0]);
...
public:
    ...
    static std::string MSToString(unsigned long time) {
        ...
        seconds -= minutes * 60;
        if (minutes < 60)
            return stringify(minutes) + ":" + (seconds < 10 ? "0" : "") +
                   stringify(seconds);
        unsigned long hours = minutes / 60;
        minutes %= 60;
        return stringify(hours) + ":" + (minutes < 10 ? "0" : "") +
               stringify(minutes) + ":" + (seconds < 10 ? "0" : "") +
               stringify(seconds);
    }
